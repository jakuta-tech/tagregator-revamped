...
function tagregator_revamped_get_flickr_posts( $the_term ) {
	...
	if ( 'yes' == $tr_options['tagregator_revamped_flickr_enabled'] ) {
		...
		$url = TAGREGATOR_REVAMPED_FLICKR_API . '?method=flickr.photos.search&tags=' . $the_term . '&per_page=20&min_upload_date=' . $min_date . '&extras=date_upload,description,owner_name,url_n,url_l,icon_farm,icon_server&format=json&nojsoncallback=1&api_key=' . $tr_options['tagregator_revamped_flickr_key'];
